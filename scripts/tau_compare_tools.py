...
def compare_es(id,era,dms=None,gms=None,tag="",verb=0):
  ...
  if "DeepTau" in id:
    ...
    idjet  = id
    idele  = "DeepTau2017v2p1VSe"
  oldtool1 = TauESTool(era,idjet,path=oldata) #,verbose=True
  oldtool2 = TauFESTool(era,idele,path=oldata) #,verbose=True
  oldmeth1 = lambda a: oldtool1.getTES(*a)
  oldmeth2 = lambda a: oldtool2.getFES(*a)
  jname    = f"data/tau/new/tau_es_dm_{era}{tag}.json"
...
def compare_sfs(id,era,xvar='pt',dms=None,gms=None,wps=None,tag="",verb=0):
  ...
  if xvar=='eta':
    jname   = f"data/tau/new/tau_sf_eta_{id}_{era}{tag}.json"
  else:
    ...
    for gm in gms:
      ...
      for x in xbins:
        if 'pt' in xvar:
          ...
          args1  = (x,gm,'All')
          args2  = (x,gm,wp) # (eta,gm,wp,syst)
          largs2 = tuple() # last arguments (after syst)
...
def main(args):
  ...
  for id in tids:
    for era in eras:
      ...
      for xvar in xvars:
        compare_sfs(id,era,xvar=xvar,dms=dms,gms=gms,wps=wps,tag=tag,verb=verbosity)
...
if __name__ == '__main__':
  ...
  parser = ArgumentParser(prog="tau_compare_tools.py",description=description,epilog="Good luck!")
  ...
  parser.add_argument('-w', '--wp',       dest='wps', nargs='+', help="WPs" )
